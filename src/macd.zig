...
const EMA = @import("trend.zig").EMA;
const Indicator = @import("base.zig").Indicator;

/// Moving Average Convergence/Divergence helps to identify price
/// trends, measure trend momentum, and identify entry points for buying
/// or selling. It is calculated based on two different period
/// exponential moving averages, usually of 26 and 12 period. The
/// indicator is the exponential moving average of that difference.
///
/// MACD has three fields:
///
/// * `macd`: it is properly the MACD value
/// * `signal`: the difference between exponential moving averages for the
/// long and short periods.
/// * `hist`: the distance between macd and its signal
///
/// Usually, hist is used to identify peaks of bullish of bearish
/// momentum, and to generate overbought or oversold trade signals.
pub fn MACD(short_period: usize, long_period: usize, diff_period: usize, smoothing: f64, mem_size: usize) type {
    return struct {
        const Result = struct {
            macd: f64 = nan,
            signal: f64 = nan,
        ...
        /// Results for MACD are stored here.
        data: Indicator(Result, Result{}, mem_size) =
            Indicator(Result, Result{}, mem_size){},
        /// Short-period trace.
        short_ema: EMA(short_period, smoothing, 1) =
            EMA(short_period, smoothing, 1){},
        /// Long-period trade.
        long_ema: EMA(long_period, smoothing, 1) =
            EMA(long_period, smoothing, 1){},
        /// The macd value is tracked here.
        diff_ema: EMA(diff_period, smoothing, 1) =
            EMA(diff_period, smoothing, 1){},
        ...
        pub fn update(self: *@This(), value: f64) void {
            ...
            if (self.counter >= self.start) {
                ...
                self.data.push(Result{ .macd = diff, .signal = self.diff_ema.curr(), .hist = diff - self.diff_ema.curr() });
            } else {
...
test "MACD" {
    ...
    for (0..ts.len, ts) |i, value| {
        ...
        if (i < test_pos) {
            ...
            try std.testing.expectApproxEqAbs(macd.curr().signal, res[i - test_pos], 1e-3);
        } else {
            try std.testing.expectApproxEqAbs(macd.curr().signal, res[i - test_pos], 1e-3);
            try std.testing.expectApproxEqAbs(macd.get(1).signal, res[i - test_pos - 1], 1e-3);
        }
    }
}
